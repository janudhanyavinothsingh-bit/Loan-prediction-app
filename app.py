...
if st.button("Predict Loan Status"):
    ...
    if cibil_score < 500:
        st.error("❌ Loan Not Approved (Reason: Low CIBIL Score)")
        decision = "Not Approved (Low CIBIL)"
    elif income_annum < 5000:
        st.error("❌ Loan Not Approved (Reason: Low Income)")
        decision = "Not Approved (Low Income)"
    elif proba[1]*100 > threshold:
        st.success("✅ Loan Approved")
        decision = "Approved"
    else:
        st.error("❌ Loan Not Approved (Reason: Probability below threshold)")
